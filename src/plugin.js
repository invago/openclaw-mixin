/**
 * Mixin Messenger 插件主文件
 * 参考 Openclaw 飞书和 Telegram 插件实现
 */
...
class MixinPlugin {
  ...
  async start() {
    ...
    try {
      ...
      this.server = this.app.listen(port, () => {
        console.log(`[${this.name}] Webhook 服务器启动，端口：${port}`);
        console.log(`[${this.name}] Webhook 地址：http://localhost:${port}/webhook/mixin`);
      });
  ...
  async stop() {
    ...
    if (this.server) {
      await new Promise((resolve) => {
        this.server.close(() => {
          console.log(`[${this.name}] Webhook 服务器已关闭`);
          resolve();
        });
      });
    }
   ...
   */
  setupRoutes() {
    // Webhook 速率限制：每 IP 每分钟最多 60 个请求
    ...
    this.app.get('/info', (req, res) => {
      res.json({
        ...
        version: this.version,
        description: '将 Openclaw AI 助手接入 Mixin Messenger 平台',
  ...
  async handleWebhook(req, res) {
    try {
      ...
      console.error(`[${this.name}] Webhook 处理失败:`, error);
