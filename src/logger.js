...
class Logger {
  ...
  sanitizeMeta(meta) {
    const sensitive = [
      'password', 'secret', 'token', 'key', 'privateKey', 'apiKey',
      'authorization', 'cookie', 'credential', 'private', 'credential'
    ];
    const sanitized = {};
  ...
  sanitizeMessage(message) {
    ...
    return message
      .replace(/[a-f0-9]{64,}/gi, '[HASH_HIDDEN]') // SHA256 等哈希
      .replace(/[A-Za-z0-9+/]{40,}={0,2}/g, '[BASE64_HIDDEN]') // Base64 编码
      .replace(/-----BEGIN [A-Z ]+-----[\s\S]*?-----END [A-Z ]+-----/g, '[PEM_KEY_HIDDEN]') // PEM 密钥
      .replace(/\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/gi, '[UUID]') // UUID
      .replace(/\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/gi, '[MIXIN_ID]'); // Mixin 用户 ID
  }
